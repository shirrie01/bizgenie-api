...
describe("campaign generation composed authorization", () => {
  ...
  it("returns a truthful quality rejection and logs safe validation reasons", async () => {
    ...
    const diagnostic = JSON.parse(warnings[0].message);
    assert.equal(diagnostic.event, "campaign_strategy_validation_rejected");
    assert.equal(diagnostic.code, "CAMPAIGN_STRATEGY_REJECTED");
    assert.equal(diagnostic.name, "CampaignStrategyValidationError");
    assert.equal(diagnostic.rejection_stage, "selected_strategy_validation");
    assert.match(diagnostic.generation_job_id, /^[0-9a-f-]{36}$/);
    assert.ok(diagnostic.reasons.includes("three to five strategy candidates are required"));
    assert.equal(JSON.stringify(response.body).includes("selected_strategy_validation"), false);
    assert.equal(JSON.stringify(response.body).includes("generation_job_id"), false);
  });
});
