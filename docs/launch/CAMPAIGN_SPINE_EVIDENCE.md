...
| [Media schema](../../src/media/schema.js), [repository](../../src/media/repository.js), [storage](../../src/media/storage.js) | Private server-owned bucket/key; active owned assets; generation-reference and campaign preview/publication rights are narrow server-owned allowlists; no customer export endpoint | Link immutable IDs, revalidate status and required campaign right; don't invent upload/publication rights or expose storage locations; I-F supplies authenticated export |
